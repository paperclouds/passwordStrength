...
@implementation NSString (HJCore)
#pragma mark - 正则匹配
/**
 *  匹配Email
 *
 ...
 */
- (BOOL)hjempty {
    if (self == nil || self == NULL || [self isKindOfClass:[NSNull class]] || [[self stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]] length]==0 || [self length] == 0 || [self isEqualToString:@"(null)"]){
        return YES;
    }
    return NO;
}
...
- (NSArray *)stringToArry{
    ...
    NSMutableArray *childArray=[NSJSONSerialization JSONObjectWithData:nsData options:NSJSONReadingMutableContainers error:nil];
